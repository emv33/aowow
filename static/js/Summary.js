...
Summary.groupOver = function(group, e) {
    ...
    for (var i = 0, len = group.length; i < len; ++i) {
        ...
        if (g_items[itemId]) {
            ...
            buff += '<span class="icontiny" style="background: url(' + Icon.url(g_items[itemId].icon) + ') left center / 15px 15px no-repeat">' + g_items[itemId]['name_' + Locale.getName()] + '</span>';
            buff += '</div></td>';
...
Summary.socketOver = function(gems, e) {
    ...
    for (var i in gems) {
        if (g_gems[i]) {
            ...
            buff += '<span class="icontiny" style="background: url(' + Icon.url(g_gems[i].icon) + ') left center / 15px 15px no-repeat">' + g_gems[i].name + '</span>';
            buff += '</div></td>';

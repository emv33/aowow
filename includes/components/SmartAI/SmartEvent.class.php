...
class SmartEvent
{
    ...
    public const int EVENT_UPDATE_IC               = 0;     // In combat.
    public const int EVENT_UPDATE_OOC              = 1;     // Out of combat.
    public const int EVENT_HEALTH_PCT              = 2;     // Health Percentage
    public const int EVENT_MANA_PCT                = 3;     // Mana Percentage
    public const int EVENT_AGGRO                   = 4;     // On Creature Aggro
    public const int EVENT_KILL                    = 5;     // On Creature Kill
    public const int EVENT_DEATH                   = 6;     // On Creature Death
    public const int EVENT_EVADE                   = 7;     // On Creature Evade Attack
    public const int EVENT_SPELLHIT                = 8;     // On Creature/Gameobject Spell Hit
    public const int EVENT_RANGE                   = 9;     // On Target In Range
    public const int EVENT_OOC_LOS                 = 10;    // On Target In Distance Out of Combat
    public const int EVENT_RESPAWN                 = 11;    // On Creature/Gameobject Respawn
    public const int EVENT_TARGET_HEALTH_PCT       = 12;    // [DEPRECATED] On Target Health Percentage
    public const int EVENT_VICTIM_CASTING          = 13;    // On Target Casting Spell
    public const int EVENT_FRIENDLY_HEALTH         = 14;    // [DEPRECATED] On Friendly Health Deficit
    public const int EVENT_FRIENDLY_IS_CC          = 15;    //
    public const int EVENT_FRIENDLY_MISSING_BUFF   = 16;    // On Friendly Lost Buff
    public const int EVENT_SUMMONED_UNIT           = 17;    // On Creature/Gameobject Summoned Unit
    public const int EVENT_TARGET_MANA_PCT         = 18;    // [DEPRECATED] On Target Mana Percentage
    public const int EVENT_ACCEPTED_QUEST          = 19;    // On Target Accepted Quest
    public const int EVENT_REWARD_QUEST            = 20;    // On Target Rewarded Quest
    public const int EVENT_REACHED_HOME            = 21;    // On Creature Reached Home
    public const int EVENT_RECEIVE_EMOTE           = 22;    // On Receive Emote.
    public const int EVENT_HAS_AURA                = 23;    // On Creature Has Aura
    public const int EVENT_TARGET_BUFFED           = 24;    // On Target Buffed With Spell
    public const int EVENT_RESET                   = 25;    // After Combat, On Respawn or Spawn
    public const int EVENT_IC_LOS                  = 26;    // On Target In Distance In Combat
    public const int EVENT_PASSENGER_BOARDED       = 27;    //
    public const int EVENT_PASSENGER_REMOVED       = 28;    //
    public const int EVENT_CHARMED                 = 29;    // On Creature Charmed
    public const int EVENT_CHARMED_TARGET          = 30;    // [DEPRECATED] On Target Charmed
    public const int EVENT_SPELLHIT_TARGET         = 31;    // On Target Spell Hit
    public const int EVENT_DAMAGED                 = 32;    // On Creature Damaged
    public const int EVENT_DAMAGED_TARGET          = 33;    // On Target Damaged
    public const int EVENT_MOVEMENTINFORM          = 34;    // WAYPOINT_MOTION_TYPE = 2,  POINT_MOTION_TYPE = 8
    public const int EVENT_SUMMON_DESPAWNED        = 35;    // On Summoned Unit Despawned
    public const int EVENT_CORPSE_REMOVED          = 36;    // On Creature Corpse Removed
    public const int EVENT_AI_INIT                 = 37;    //
    public const int EVENT_DATA_SET                = 38;    // On Creature/Gameobject Data Set, Can be used with SMART_ACTION_SET_DATA
    public const int EVENT_WAYPOINT_START          = 39;    // [DEPRECATED] On Creature Waypoint ID Started
    public const int EVENT_WAYPOINT_REACHED        = 40;    // On Creature Waypoint ID Reached
    public const int EVENT_TRANSPORT_ADDPLAYER     = 41;    // [RESERVED]
    public const int EVENT_TRANSPORT_ADDCREATURE   = 42;    // [RESERVED]
    public const int EVENT_TRANSPORT_REMOVE_PLAYER = 43;    // [RESERVED]
    public const int EVENT_TRANSPORT_RELOCATE      = 44;    // [RESERVED]
    public const int EVENT_INSTANCE_PLAYER_ENTER   = 45;    // [RESERVED]
    public const int EVENT_AREATRIGGER_ONTRIGGER   = 46;    //
    public const int EVENT_QUEST_ACCEPTED          = 47;    // [RESERVED] On Target Quest Accepted
    public const int EVENT_QUEST_OBJ_COMPLETION    = 48;    // [RESERVED] On Target Quest Objective Completed
    public const int EVENT_QUEST_COMPLETION        = 49;    // [RESERVED] On Target Quest Completed
    public const int EVENT_QUEST_REWARDED          = 50;    // [RESERVED] On Target Quest Rewarded
    public const int EVENT_QUEST_FAIL              = 51;    // [RESERVED] On Target Quest Field
    public const int EVENT_TEXT_OVER               = 52;    // On TEXT_OVER Event Triggered After SMART_ACTION_TALK
    public const int EVENT_RECEIVE_HEAL            = 53;    // On Creature Received Healing
    public const int EVENT_JUST_SUMMONED           = 54;    // On Creature Just spawned
    public const int EVENT_WAYPOINT_PAUSED         = 55;    // On Creature Paused at Waypoint ID
    public const int EVENT_WAYPOINT_RESUMED        = 56;    // On Creature Resumed after Waypoint ID
    public const int EVENT_WAYPOINT_STOPPED        = 57;    // On Creature Stopped On Waypoint ID
    public const int EVENT_WAYPOINT_ENDED          = 58;    // On Creature Waypoint Path Ended
    public const int EVENT_TIMED_EVENT_TRIGGERED   = 59;    //
    public const int EVENT_UPDATE                  = 60;    //
    public const int EVENT_LINK                    = 61;    // Used to link together multiple events as a chain of events.
    public const int EVENT_GOSSIP_SELECT           = 62;    // On gossip clicked (gossip_menu_option335).
    public const int EVENT_JUST_CREATED            = 63;    //
    public const int EVENT_GOSSIP_HELLO            = 64;    // On Right-Click Creature/Gameobject that have gossip enabled.
    public const int EVENT_FOLLOW_COMPLETED        = 65;    //
    public const int EVENT_EVENT_PHASE_CHANGE      = 66;    // [DEPRECATED] On event phase mask set
    public const int EVENT_IS_BEHIND_TARGET        = 67;    // [DEPRECATED] On Creature is behind target.
    public const int EVENT_GAME_EVENT_START        = 68;    // On game_event started.
    public const int EVENT_GAME_EVENT_END          = 69;    // On game_event ended.
    public const int EVENT_GO_LOOT_STATE_CHANGED   = 70;    //
    public const int EVENT_GO_EVENT_INFORM         = 71;    //
    public const int EVENT_ACTION_DONE             = 72;    //
    public const int EVENT_ON_SPELLCLICK           = 73;    //
    public const int EVENT_FRIENDLY_HEALTH_PCT     = 74;    //
    public const int EVENT_DISTANCE_CREATURE       = 75;    // On creature guid OR any instance of creature entry is within distance.
    public const int EVENT_DISTANCE_GAMEOBJECT     = 76;    // On gameobject guid OR any instance of gameobject entry is within distance.
    public const int EVENT_COUNTER_SET             = 77;    // If the value of specified counterID is equal to a specified value
    public const int EVENT_SCENE_START             = 78;    // [RESERVED] don't use on 3.3.5a
    public const int EVENT_SCENE_TRIGGER           = 79;    // [RESERVED] don't use on 3.3.5a
    public const int EVENT_SCENE_CANCEL            = 80;    // [RESERVED] don't use on 3.3.5a
    public const int EVENT_SCENE_COMPLETE          = 81;    // [RESERVED] don't use on 3.3.5a
    public const int EVENT_SUMMONED_UNIT_DIES      = 82;    //
    public const int EVENT_ON_SPELL_CAST           = 83;    // On Spell::cast
    public const int EVENT_ON_SPELL_FAILED         = 84;    // On Unit::InterruptSpell
    public const int EVENT_ON_SPELL_START          = 85;    // On Spell::prapare
    public const int EVENT_ON_DESPAWN              = 86;    // On before creature removed
    public const int EVENT_SEND_EVENT_TRIGGER      = 87;    // [RESERVED] UNUSED NEEDS CHERRYPICK
    public const int EVENT_AREATRIGGER_EXIT        = 88;    // [RESERVED] don't use on 3.3.5a
    public const int EVENT_ON_AURA_APPLIED         = 89;    //
    public const int EVENT_ON_AURA_REMOVED         = 90;    //

    public const int FLAG_NO_REPEAT        = 0x0001;
    public const int FLAG_DIFFICULTY_0     = 0x0002;
    public const int FLAG_DIFFICULTY_1     = 0x0004;
    public const int FLAG_DIFFICULTY_2     = 0x0008;
    public const int FLAG_DIFFICULTY_3     = 0x0010;
    public const int FLAG_DEBUG_ONLY       = 0x0080;
    public const int FLAG_NO_RESET         = 0x0100;
    public const int FLAG_WHILE_CHARMED    = 0x0200;
    public const int FLAG_ALL_DIFFICULTIES = self::FLAG_DIFFICULTY_0 | self::FLAG_DIFFICULTY_1 | self::FLAG_DIFFICULTY_2 | self::FLAG_DIFFICULTY_3;
    public const int FLAG_VALIDATE         = self::FLAG_NO_REPEAT | self::FLAG_DEBUG_ONLY | self::FLAG_NO_RESET | self::FLAG_WHILE_CHARMED | self::FLAG_ALL_DIFFICULTIES;

    private const string EVENT_CELL_TPL = '[tooltip name=e-#rowIdx#]%1$s[/tooltip][span tooltip=e-#rowIdx#]%2$s[/span]';

    /**
     * Event parameter definitions.
     *
     * The first five values define the event parameters. Each value is either
     * a positive integer type or an array containing a validation function,
     * parameter index, and extra parameter. The sixth value is the error class.
     *

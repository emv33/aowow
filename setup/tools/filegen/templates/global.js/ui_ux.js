...
function g_getMoneyHtml(money, side, costItems, costCurrency, achievementPoints)
{
    ...
    if (costItems != null)
    {
        for (var i = 0; i < costItems.length; ++i)
        {
            ...
            html += '<a href="?item=' + itemId + '" class="moneyitem" style="' + Icon.tinyBg(icon) + '">' + count + '</a>';
        }
    }
    ...
    if (costCurrency != null)
    {
        for (var i = 0; i < costCurrency.length; ++i)
        {
            ...
            if (currencyId == 103)                          // arena
                ...
                html += '<a href="?currency=' + currencyId + '" class="icontinyr tip q1" onmouseover="Listview.funcBox.moneyCurrencyOver(' + currencyId + ', ' + count + ', event)" onmousemove="$WH.Tooltip.cursorUpdate(event)" onmouseout="$WH.Tooltip.hide()" style="' + Icon.tinyBg(icon[0]) + '">' +  count + '</a>';
            // aowow: custom end

...
Mapper.prototype = {
    ...
    setZones: function(div, zones)
    {
        ...
        var zoneLinks = function(self, container, zoneList, zoneTypes)
        {
            ...
            {
                if (i > 0)
                    span.append(i == zoneList.length-1 ? LANG.and : LANG.comma);

                var entry = null;
                if (self.objectives[zoneList[i][0]].mappable > 0)
                {
					entry = $('<a/>', { href: 'javascript:;', text: self.objectives[zoneList[i][0]].zone });
                    entry.click(function(link, zone) {
                        self.update({ zone: zone });
                        g_setSelectedLink(link, 'mapper');
                    }.bind(self, entry[0], zoneList[i][0]));
                    entry.isLink = true;
                }
                else
                {
                    entry = $('<a/>', { href: '?zone=' + zoneList[i][0], text: self.objectives[zoneList[i][0]].zone });
                    g_addTooltip(entry[0], LANG.tooltip_zonelink);
                }
                ...
                if (zones.length > 1)
                {
                    ...
                    if (types.start && types.end)
                    {
						...
						entry.css('background-image', 'url(' + Icon.url('quest_startend', null, true) + ')');
						entry.css('padding-left', '20px');
                    }
                    else if (types.start)
                    {
						entry.addClass('icontiny');
						entry.css('background-image', 'url(' + Icon.url('quest_start', null, true) + ')');
						entry.css('padding-left', '14px');
                    }
                    else if (types.end)
                    {
						entry.addClass('icontiny');
						entry.css('background-image', 'url(' + Icon.url('quest_end', null, true) + ')');
		...
		var h3 = $('<h3/>', { text: LANG.mapper_relevantlocs });
		div.append(h3);

        if (zones.length == 1 && this.missing == 0)
        {
			var span = $('<span/>', { html: LANG.mapper_entiretyinzone.replace('$$', '<b>' + this.objectives[zones[0][0]].zone + '</b>.') });
			div.append(span);
        ...
        else if (this.missing > 0)
        {
            ...
            else if (endObj) // objectives and ends in x, starts in y
            {
				...
				primaryLink = zoneLinks(this, span, objZones, typesByZone);
				span.append(parts[1]);
                secondaryLink = zoneLinks(this, span, startZones, typesByZone);
				span.append(parts[2]);
            }
    ...
    {
        this.tempWidth = w; this.tempHeight = h; this.updateMap(true);
    },
    ...
    {
        this.zoom = zoom; this.tempWidth = this.tempHeight = null; this.updateMap(noScroll);
    },
    ...
    updateMap: function(noScroll)
    {
        ...
        // aowow - wrapped in an if, as this.floorPins[level] may be undefined
        // which causes jQuery to default to document as search root
        // which then gets all lines for all Mappers everywhere
        if (this.floorPins[level])
        {
